...
def test(args, device, save_dir):
    """Run test-mode evaluation using the unified evaluator."""
    import evaluate
    ...
    return evaluate.run_evaluation(
        ...
        shots=args.test_num_support,
        use_nearest_slice=args.use_nearest_slice,
        eval_mode='test',
        data_root=os.environ.get("ADASEMSEG_DATA_ROOT", "./data"),
        output_dir=save_dir,
        device=args.device,
        batch_size=1,
...
def main():
    ...
    parser.add_argument("--classes", nargs="+", default=None,
                        help="Override the source/target class names used for training/validation "
                             "(e.g., f3_facies_data_inline parihaka_facies_data_inline). "
                             "If omitted, the classes list from config/local_config.py is used.")
    ...
    if img_enc_type=='resnet':
        if args.random_init:
            ...
            print("Randomly initializing the ResNet50 image encoder (random-init ablation).")
        else:
            resnet = models.resnet50(pretrained=args.image_net_stat)
        resnet.fc = nn.Identity()

        if args.img_enc_checkpoint and not args.random_init:
    ...
    if args.train:
        ...
        train_aug_indices = aug_name_to_indices[args.augmentation]

        seismic_seg_data_loader = Seismic_Segmentation_Task(classes, data_info, patch_size=patch_size, k_shot=k_shot,
                                                            train_batch_size=train_batch_size, val_batch_size=val_batch_size,
                                                            use_nearest_slice=args.use_nearest_slice,
                                                            train_aug_indices=train_aug_indices, debug=False)
        ...
        run_id = args.run_id
        test_save_dir = os.path.join(config['exp_spec'], str(k_shot)+'-shot', 'Run_' + str(run_id), 'test_results')
        os.makedirs(test_save_dir, exist_ok=True)
        test(args, device, test_save_dir)
