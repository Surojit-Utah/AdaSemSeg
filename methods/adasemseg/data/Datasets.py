...
class Seismic_Segmentation_Task():
    ...
    def __init__(self, classes, data_info, patch_size, k_shot=None, train_batch_size=10, val_batch_size=10,
                 use_nearest_slice=False, train_aug_indices=None, debug=False):

        self.traindataset = CustomDataset(classes, data_info, patch_size, train_batch_size, k_shot=k_shot, mode='train',
                                          use_nearest_slice=False, train_aug_indices=train_aug_indices, debug=debug)
        ...
        self.validdataset = CustomDataset(classes, data_info, patch_size, val_batch_size, k_shot=k_shot, mode='val',
                                          use_nearest_slice=use_nearest_slice, debug=debug)
...
class CustomDataset(Dataset):
    ...
    def __init__(self, classes, data_info, patch_size, batch_size, k_shot=None, mode=None,
                 use_nearest_slice=False, train_aug_indices=None, debug=False):
        ...
        # Augmentation indices used during training. Default: all augmentations (1-4).
        # Use [] or None to disable augmentations; use [1] for only RandomRotate, etc.
        if train_aug_indices is None:
            train_aug_indices = list(range(1, 5))
        self.train_aug_indices = train_aug_indices
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            # If no augmentations are configured, apply only resize + tensor conversion.
            if len(self.train_aug_indices) == 0:
                query_aug = None
            else:
                query_aug = self.aug_dict[query_aug_index]

            for s_i, s_cm, s_sm, s_aug in zip(support_imgs, support_masks, support_split_masks, support_aug_index_list):
                if query_aug is None:
                    cur_trans = Compose([Resize(self.patch_size), ToTensor()])
                else:
                    cur_trans = Compose([self.aug_dict[s_aug],
    ...
    def sample_episode(self, idx):
        ...
        if self.mode == 'train':
            if len(self.train_aug_indices) == 0:
                query_aug_index = 0  # placeholder; not used
            else:
                query_aug_index = random.choice(self.train_aug_indices)
        ...
        ##############################################
        if self.mode == 'val' and self.use_nearest_slice:
            # Nearest-slice support selection: pick the k_shot train slices closest
            # to the query slice (used for Parihaka in the paper).
            valid_indices = np.array(valid_indices)
            distances = np.abs(valid_indices - query_image_index)
            nearest_order = np.argsort(distances)
            support_sample_index = valid_indices[nearest_order[:self.k_shot]].tolist()
        elif len(valid_indices) < self.k_shot:
        ...
        for set_support_sample_index in range(self.k_shot):
            ...
            if self.mode == 'train':
                if len(self.train_aug_indices) == 0:
                    support_aug_index = 0  # placeholder; not used
                else:
                    support_aug_index = random.choice(self.train_aug_indices)
