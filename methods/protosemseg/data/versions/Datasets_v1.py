...
class CustomDataset(Dataset):
    ...
    def __init__(self, classes, data_info, patch_size, batch_size, k_shot=None, mode=None, debug=False):
        ...
        # validation
        # Remember the number of slices in the validation set is relatively small
        self.val_cushion_sample_count = 2       # Skip slices around the selected query image
        self.val_import_sample_count = 5        # Number of images after the *cushion* indices
        self.val_skip_image_indices = 1         # Skip intermediate slices for the support images
    ...
    def sample_episode(self, idx):
        ...
        # Used only for the validation data
        # As the training data is sampled from the selected slices (say 10, 20, or 50 slices)
        #####################################################################################
        min_slice_query_class = self.slice_indices[query_class][0]
        max_slice_query_class = self.slice_indices[query_class][-1]

        # indices to be used for sampling support examples
        if self.mode=='train':
            # left indices
        ...
        if self.mode=='val':
            # left indices
            min_valid_left_index = max(query_image_index - self.val_cushion_sample_count - self.val_import_sample_count, min_slice_query_class)
            max_valid_left_index = max(query_image_index - self.val_cushion_sample_count, min_slice_query_class)
            valid_left_indices = np.arange(min_valid_left_index, max_valid_left_index + 1, self.val_skip_image_indices)

            # right indices
            min_valid_right_index = min(query_image_index + self.val_cushion_sample_count, max_slice_query_class)
            max_valid_right_index = min(query_image_index + self.val_cushion_sample_count + self.val_import_sample_count, max_slice_query_class)
            valid_right_indices = np.arange(min_valid_right_index, max_valid_right_index + 1, self.val_skip_image_indices)
        ...
        if self.debug:
            ...
            print("Index range              : " + str(min_slice_query_class) + " , " + str(max_slice_query_class))
...
if __name__ == '__main__':
    ...
    #
    #                 support_split_mask_path = os.path.join(_split_mask_path, 'Support_Batch_' + str(b + 1) + '.png')
    #                 show_combined_images(support_images_np[b], support_split_anno_np[b, :, class_index], support_split_mask_path)
    #
    #         query_images = data['query_image'].cpu().detach()
    #         query_images_np = query_images.numpy().transpose(0, 1, 3, 4, 2)
    #         query_anno = data['query_segmentation'].cpu().detach()
    ...
    #
    #                 query_split_mask_path = os.path.join(_split_mask_path, 'Query_Batch_' + str(b + 1) + '.png')
    #                 show_combined_images(query_images_np[b], query_split_anno_np[b, :, class_index], query_split_mask_path)
    #
    #     else:
