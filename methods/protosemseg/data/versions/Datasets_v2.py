...
class CustomDataset(Dataset):
    ...
    def sample_episode(self, idx):
        ...
        # For the training data (say 10, 20, or 50 slices), examples other than the query image are used as support images
        #####################################################################################

        # indices to be used for sampling support examples
        if self.mode=='train':
            query_class_indices = self.slice_indices[query_class]
            valid_indices = np.sort(np.array(list(set(query_class_indices).difference([query_image_index]))))
