...
class Deep_GP_Trainer:
    ...
    def __init__(self, tensorboard_writer, model, loss, optimizer, dataset_n_loader,
        ...
        # class weights for the Cross entropy loss
        # self.class_weights['parihaka'] = [0.5, 0.5, 0.5, 0.5, 0.5, 0.5]
        # self.class_weights['penobscot'] = [0.5, 0.5, 0.8, 0.6, 0.4, 0.5, 0.3]
        # self.class_weights['f3'] = [0.5, 0.7, 0.5, 0.7, 0.7, 0.7]

        self.class_weights_option = 1

        # Option 1
        if self.class_weights_option==1:
        ...
        # Option 2
        if self.class_weights_option==2:
            self.class_weights['parihaka'] = [0.85, 0.578, 0.959, 0.712, 0.99, 0.911]
            # self.class_weights['parihaka'] = [0.7, 0.5, 0.95, 0.6, 0.95, 0.8]
            self.class_weights['penobscot'] = [0.905, 0.882, 0.977, 0.971, 0.726, 0.851, 0.689]
            self.class_weights['f3'] = [0.719, 0.881, 0.514, 0.934, 0.967, 0.985]

        # Processing of the predictions for visualization
        self.predict_act = torch.nn.Softmax2d()
        self._seg_threshold = 0.5
        self._cosine_scaler = 20
    ...
    def _run_epoch(self, mode, data_loader):
        ...
        for iter_num, data in enumerate(data_loader):
            ...
            S_B, S_S, S_C, S_H, S_W = self._support_images.size()
            encoder_input = self._support_images.view(S_B*S_S, S_C, S_H, S_W)
            support_features = self._model.image_encoder(encoder_input)
            ...
            Q_B, Q_S, Q_C, Q_H, Q_W = self._query_image.size()
            encoder_input = self._query_image.view(Q_B*Q_S, Q_C, Q_H, Q_W)
            query_features = self._model.image_encoder(encoder_input)
            encoded_query_image_prototype = query_features['f32']

            for class_index, class_label in enumerate(self.class_indices[class_name]):
                cur_fg_mask = self._support_split_masks[:, :, class_label - 1]
                cur_bg_mask = 1 - self._support_split_masks[:, :, class_label - 1]
                ...
                # Similarity scores
                fg_similarity_score = F.cosine_similarity(encoded_query_image_prototype, fg_masked_fts[None, ..., None, None], dim=1) * self._cosine_scaler
                bg_similarity_score = F.cosine_similarity(encoded_query_image_prototype, bg_masked_fts[None, ..., None, None], dim=1) * self._cosine_scaler
                ...
                similarity_score = self.predict_act(similarity_score)

                # query prediction
                query_features['f32'] = similarity_score # fed as input to the decoder
                query_pred = self._model.image_decoder(query_features)
                ...
                if self._loss=='weighted_bce':
                    ...
                    if self.class_weights_option==1:
                        ...
                        WCE_loss_per_class = nn.CrossEntropyLoss(weight=torch.tensor([1.0/pos_label_weight, pos_label_weight]).to(self._device))(query_pred, groundtruth)
                    elif self.class_weights_option==2:
                        WCE_loss_per_class = nn.CrossEntropyLoss(weight=torch.tensor([1.0 - pos_label_weight, pos_label_weight]).to(self._device))(query_pred, groundtruth)
