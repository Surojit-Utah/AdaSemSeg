...
def main():
    ...
    parser.add_argument("--eval_mode", type=str, default='val', required=True)
    ...
    class_name = config['classes'][0]
    data_info = config['data_info']

    # Dataset details
    if args.source_class is not None:
        source_class = args.source_class
        if source_class not in config['data_info'].keys():
    ...
    if img_enc_type=='resnet':
        ...
        fss_decoder_obj = DGP_resnet_unet.Image_Decoder()
        fss_learner_obj = DGP_resnet_unet.Segmentation_Network(image_encoder=img_encoder_obj, upsampler=fss_decoder_obj)
    ...
    if args.checkpoint_dir:
        ...
        if best_model:
            exp_spec_path = os.path.join(config['exp_spec'], source_class, str(k_shot) + '-shot',
                                         'Run_' + str(run_id) + '_best_model', eval_mode)
            os.makedirs(exp_spec_path, exist_ok=True)
            visualization_path = os.path.join(config['visualization_path'], source_class, str(k_shot) + '-shot',
                                              'Run_' + str(run_id) + '_best_model', eval_mode)
            os.makedirs(visualization_path, exist_ok=True)
            results_path = os.path.join(config['results_path'], source_class, str(k_shot) + '-shot',
                                        'Run_' + str(run_id) + '_best_model', eval_mode)
        ...
        assert type(fss_learner_obj).__name__ == checkpoint['net_type']
        fss_learner_obj.load_state_dict(checkpoint['net'])
    ...
    test_loader = Dataset_Loader(source_class, data_info, eval_mode=eval_mode, batch_size=batch_size, k_shot=k_shot)

    # Calling the model evaluator
    visualize_model_pred = bool(args.visualize)
    cur_save_dir = visualization_path
    iou_score_by_class, iou_list, tricky_examples, metric_scores_dict, eval_time_stats = test(fss_learner_obj, device, cur_save_dir, test_loader, visualize_model_pred)
    ...
    sel_summary_fptr.write('Average IoU for ' + str(args.shots) + '-shot learning using Method1 is ' + str(np.round(method1_avg, 3)) +
