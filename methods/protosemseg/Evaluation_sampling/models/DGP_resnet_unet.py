...
class Image_Decoder(nn.Module):
    ...
    def __init__(self, image_features=64, n_class=2, affine=True):
        super().__init__()
        ...
        self.upsample1 = nn.ConvTranspose2d(2, self.image_features*16, kernel_size=2, stride=2)              # (B, 64, H//16, H//16)
    ...
    def forward(self, encoded_image_features):
        ...
        x = self.upsample5(x)                                           # (B, 64, H//1, H//1)
        out = self.conv_last(x)                                         # (B, 01, H//1, H//1)
