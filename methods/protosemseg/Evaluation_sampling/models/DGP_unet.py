...
class Image_Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        features['f1'] = conv1 = self.dconv_down1(x)        # (B*N, 32, H, W)
        x = self.downsample_1(conv1)                        # (B*N, 32, H//2, W//2)

        features['f2'] = conv2 = self.dconv_down2(x)        # (B*N, 64, H//2, W//2)
        x = self.downsample_2(conv2)                        # (B*N, 64, H//4, W//4)

        features['f3'] = conv3 = self.dconv_down3(x)        # (B*N, 128, H//4, W//4)
        x = self.downsample_3(conv3)                        # (B*N, 128, H//8, W//8)

        features['f4'] = conv4 = self.dconv_down4(x)        # (B*N, 256, H//8, W//8)
        x = self.downsample_4(conv4)                        # (B*N, 256, H//16, W//16)

        conv5 = self.dconv_down5(x)        # (B*N, 512, H//16, W//16)
        features['gp_input_16'] = conv5.view(B, N, -1, H//16, W//16)
        x = self.downsample_5(conv5)                        # (B*N, 512, H//32, W//32)

        x = self.dconv_down6(x)                             # (B*N, 1024, H//32, W//32)

        gp_input = self.last_conv(x)
        features['gp_input_32'] = gp_input.view(B, N, -1, H//32, W//32)    # (B*N, 512, H//32, W//32)
...
class Mask_Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        features['f3'] = conv3 = self.dconv_down3(x)        # (B*N, 32, H//4, W//4)
        x = self.downsample_3(conv3)                        # (B*N, 32, H//8, W//8)

        features['f4'] = conv4 = self.dconv_down4(x)        # (B*N, 64, H//8, W//8)
...
class FSS_Decoder(nn.Module):
    ...
    def __init__(self, n_class=1, image_features=32, mask_features=8, covar_size=5, sigmoid=False, affine=True):
        ...
        self.gp_output_dconv1 = double_conv((self.mask_features*8 + self.covar_size**2), self.image_features*32, affine=affine)   # (B, 64, H//32, H//32)
        self.gp_output_dconv2 = double_conv((self.mask_features*8 + self.covar_size**2), self.image_features*16, affine=affine)   # (B, 64, H//32, H//32)
        self.dconv_up2 = double_conv((self.image_features*16 + self.image_features*16), self.image_features*16, affine=affine)    # (B, 64, H//16, H//16)
        self.dconv_up3 = double_conv((self.image_features*8 + self.image_features*8), self.image_features*8, affine=affine)       # (B, 64, H//8, H//8)
        self.dconv_up4 = double_conv((self.image_features*4 + self.image_features*4), self.image_features*4, affine=affine)       # (B, 32, H//4, H//4)
        self.dconv_up5 = double_conv((self.image_features*2 + self.image_features*2), self.image_features*2, affine=affine)       # (B, 16, H//2, H//2)
        self.dconv_up6 = double_conv((self.image_features*1 + self.image_features*1), self.image_features*1, affine=affine)       # (B, 08, H//1, H//1)
    ...
    def forward(self, predicted_segmentation_encodings, encoded_image_features):
        ...
        gp_output_32 = self.gp_output_dconv1(gp_output_32)              # (B*M, 1024, H//32, H//32)

        x = self.upsample1(gp_output_32)                                # (B*M, 512, H//16, H//16)
        # Convolving the output GP
        ...
        x = torch.cat([x, gp_output_16], dim=1)                         # (B*M, 512+512, H//16, H//16)
        x = self.dconv_up2(x)                                           # (B*M, 512, H//16, H//16)

        x = self.upsample2(x)                                           # (B*M, 256, H//8, H//8)
        x = torch.cat([x, encoded_image_features['f4']], dim=1)         # (B*M, 256+256, H//8, H//8)
        x = self.dconv_up3(x)                                           # (B*M, 256, H//8, H//8)

        x = self.upsample3(x)                                           # (B*M, 128, H//4, H//4)
        x = torch.cat([x, encoded_image_features['f3']], dim=1)         # (B*M, 128+128, H//4, H//4)
        x = self.dconv_up4(x)                                           # (B*M, 128, H//4, H//4)

        x = self.upsample4(x)                                           # (B*M, 64, H//2, H//2)
        x = torch.cat([x, encoded_image_features['f2']], dim=1)         # (B*M, 64+64, H//2, H//2)
        x = self.dconv_up5(x)                                           # (B*M, 64, H//2, H//2)

        x = self.upsample5(x)                                           # (B*M, 32, H//1, H//1)
        x = torch.cat([x, encoded_image_features['f1']], dim=1)         # (B*M, 32+32, H//1, H//1)
        x = self.dconv_up6(x)                                           # (B*M, 32, H//1, H//1)
