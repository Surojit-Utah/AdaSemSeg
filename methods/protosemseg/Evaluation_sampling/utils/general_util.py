...
def show_support_images(input_images, anno_input_images, support_images_path):
    ...
    for img_index in range(num_images):
        ...
        single_channel = np.expand_dims(img[:, :, 0], axis=2)
        img = np.concatenate((single_channel, single_channel, single_channel), axis=2)

        # Debug
        # img_path = os.path.join(os.path.split(support_images_path)[0], 'support_image_'+str(img_index+1)+'.png')
        ...
        # Debug
        # anno_path = os.path.join(os.path.split(support_images_path)[0], 'support_anno_'+str(img_index+1)+'.png')
        # imageio.imwrite(anno_path, anno_img[:,:,0])
        # imageio.imwrite(anno_path, anno_img)
        ...
        ax.set_aspect('equal')
        plt.axis('off')
        plt.imshow(img) # Show only a single channel
        ...
        ax.set_aspect('equal')
        plt.axis('off')
        plt.imshow(anno_img) # Show all channels. Visualization trick is applied.

    plt.savefig(support_images_path)
    plt.close(plt.gcf())
...
def show_combined_images(input_images, anno_images, save_img_path):
    ...
    for img_index in range(num_images):
        ...
        img = (img * 255.0).astype(np.uint8)
        img = np.clip(img, 0, 255)
        anno_img = (anno_img * 255.0).astype(np.uint8)
