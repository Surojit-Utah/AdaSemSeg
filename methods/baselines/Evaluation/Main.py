...
def main():
    ...
    parser.add_argument("--source_class", type=str, default=None,
                        help="Class to evaluate (if omitted, interactive prompt is used)")
    ...
    if args.checkpoint_dir:
        ...
        if best_model:
            exp_spec_path = os.path.join(config['exp_spec'], source_class,
                                         'Run_' + str(run_id) + '_best_model', eval_mode)
            os.makedirs(exp_spec_path, exist_ok=True)
            visualization_path = os.path.join(config['visualization_path'], source_class,
                                              'Run_' + str(run_id) + '_best_model', eval_mode)
            os.makedirs(visualization_path, exist_ok=True)
            results_path = os.path.join(config['results_path'], source_class,
                                        'Run_' + str(run_id) + '_best_model', eval_mode)
    ...
    iou_score_by_class, iou_list, tricky_examples, metric_scores_dict = test(seg_model_obj, device, cur_save_dir, test_loader, visualize_model_pred)
