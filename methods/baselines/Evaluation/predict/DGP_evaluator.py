...
from utils.general_util import revert_normalization, COLOR_RED, COLOR_WHITE, show_query_pred_image
...
class FSSEvaluator:
    ...
    def _get_visualization_summary(self, class_indices, images, segmentations_masks):
        ...
        for class_index in class_indices:
            ...
            single_channel_img = torch.unsqueeze(single_channel_img, dim=1)
            images = torch.cat((single_channel_img, single_channel_img, single_channel_img), dim=1)
    ...
    def _vis_query_example(self, class_name, vis_data, model, class_indices, iter_num):
        ...
        with torch.no_grad():
            query_image, query_segmentation, pred_summary = self._evaluate_episode(model, class_name, vis_data, 0, class_indices, visualize=True)
            # Input image
            query_image = query_image.cpu().detach()
            query_image_np = query_image.numpy().transpose(0, 2, 3, 1)
            # Segmentation Label
            query_anno_vis = self._get_visualization_summary(class_indices, query_image, query_segmentation)
            query_anno_vis_np = query_anno_vis.numpy().transpose(0, 2, 3, 1)
            # Prediction for IoU score
            query_pred_vis = self._get_visualization_summary(class_indices, query_image, pred_summary)
            query_pred_vis_np = query_pred_vis.numpy().transpose(0, 2, 3, 1)

            B, _, _, _ = query_image.size()
            for b in range(B):
                query_pred_path = os.path.join(self._query_path, 'Pred_Image_batch_' + str(b + 1) + '.png')
                show_query_pred_image(query_image_np[b], query_anno_vis_np[b], query_pred_vis_np[b], query_pred_path)


    def _evaluate_episode(self, model, class_name, data, episode, class_indices, visualize=False):
        ...
        query_pred = model(query_image)  # Shape : [B, C, H, W], where C varies from one dataset to the other
        pred_summary = torch.argmax(query_pred, axis=1) + 1
        ...
        iou_summary = self._get_iou_summary(class_indices, pred_summary.to(torch.int64),
                                                  torch.squeeze(query_segmentation, dim=1).to(torch.int64))
        ...
        print("Classwise IoU score  : " + str(iou_summary))
        print("Average IoU score    : " + str(np.mean(iou_summary)))
    ...
    def evaluate(self, model, dataloader, visualize=False):
        ...
        if 'f3' in dataloader.class_name:
            ...
            image_height = dataloader.train_label_vol.shape[-1]
        ...
        with torch.no_grad():
            for iter_num in range(iter_per_epoch):
                ...
                if visualize:
                    self._vis_query_example(dataloader.class_name, data, model, class_labels, iter_num+1)

                start_time = time.time()
                class_iou_scores, gt_np, pred_np = self._evaluate_episode(model, dataloader.class_name, data, iter_num, class_labels, visualize=False)
        ...
        return iou_score_by_class, iou_list, tricky_examples, metric_scores_dict