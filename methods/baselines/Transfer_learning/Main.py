...
def main():
    ...
    parser.add_argument("--classes", nargs="+", default=None,
                        help="Override the class names used for training/validation "
                             "(e.g., f3_facies_data_inline). If omitted, config/local_config.py is used.")
    ...
    if args.classes is not None:
        ...
        print(f"Overriding classes from command line: {args.classes}")
    # Training of the model parameters
    if args.train:

        # Added the run_id
        run_id = args.run_id
        config['exp_spec'] = os.path.join(config['exp_spec'], 'Train_slices_' + args.train_indices, 'Run_' + str(run_id))
        os.makedirs(config['exp_spec'], exist_ok=True)
        config['tb_dir'] = os.path.join(config['tb_dir'], 'Train_slices_' + args.train_indices, 'Run_' + str(run_id))
        os.makedirs(config['tb_dir'], exist_ok=True)
        config['visualization_path'] = os.path.join(config['visualization_path'], 'Train_slices_' + args.train_indices, 'Run_' + str(run_id))
        os.makedirs(config['visualization_path'], exist_ok=True)
        config['checkpoint_path'] = os.path.join(config['checkpoint_path'], 'Train_slices_' + args.train_indices, 'Run_' + str(run_id))
        ...
        shared_image_decoder = [param for param in img_decoder_obj.parameters()]
        class_specific_params, class_specific_param_names = [], []
        for name, param in seg_model_obj.named_parameters():
            if param.requires_grad and 'classifier' in name:
                class_specific_params.append(param)
                class_specific_param_names.append(name)
        params_image_decoder = shared_image_decoder + class_specific_params
        trainable_params = [{'params': [param for param in params_image_decoder if param.requires_grad]},
                      {'params': [param for param in params_image_encoder if param.requires_grad], 'lr': 1e-06}]

        # Optimization hyper-parameters
        num_epochs = 10
        train_batch_size = 5 # 5 for F3 facies, else 10
        val_batch_size = 40
        ...
        seismic_seg_data_loader = Seismic_Segmentation_Task(classes, data_info, patch_size=patch_size, train_indices=args.train_indices,
                                                            train_batch_size=train_batch_size, val_batch_size=val_batch_size, debug=False)

        epoch_interval = 5
        vis_epochs = [1] + np.arange(epoch_interval, num_epochs+epoch_interval, epoch_interval).tolist()
        dgp_fss_trainer = Deep_GP_Trainer(tensorboard_writer=tb_writer, model=seg_model_obj, loss='weighted_bce',
        ...
        if args.checkpoint:
            dgp_fss_trainer.load_checkpoint(args.checkpoint)
        ...
        for class_name in classes:
            ...
            exp_spec_fptr.write('Number of train slices         : ' + args.train_indices + '\n\n')
        exp_spec_fptr.write('\n')
        ...
        if not args.restart:
            dgp_fss_trainer.load_checkpoint()
            for param_group in dgp_fss_trainer._optimizer.param_groups:
                print(param_group['lr'])
