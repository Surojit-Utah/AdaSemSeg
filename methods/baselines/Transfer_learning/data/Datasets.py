...
class Seismic_Segmentation_Task():
    ...
    def __init__(self, classes, data_info, patch_size, train_indices, train_batch_size=10, val_batch_size=10, debug=False):

        self.traindataset = CustomDataset(classes, data_info, patch_size, train_batch_size, train_indices, mode='train', debug=debug)
...
class CustomDataset(Dataset):
    ...
    def __init__(self, classes, data_info, patch_size, batch_size, train_indices='all', mode=None, debug=False):
        ...
        # Same augmentation ranges as AdaSemSeg (paper Appendix B): trained with the
    ...
    def build_img_metadata(self):
        ...
        for class_name in self.classes:
            ...
            train_slices = self.train_indices
            ...
            # The first slice is ignored as it was introduced for the selection of k-support images and 1 query image in FSS,
            # requiring (k+1) images from the target volume
            # However, the same is not applicable for transfer learning
            # Thus, we use only k-slices for finetuning operation
            # This experiment was tried only on the Parihaka dataset. Thus, ignored for other class names.
            if 'parihaka' in class_name:
                labeled_indices = labeled_indices[1:]
            ...
            print('Slice indices: ' + str(labeled_indices))
