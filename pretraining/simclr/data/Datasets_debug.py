...
class Seismic_Segmentation_Task():
    ...
    def __init__(self, classes, data_info, patch_size, train_batch_size=10, val_batch_size=10, debug=False):
        ...
        # # training_sampler = RandomSampler(self.traindataset, replacement=False)
        ...
        # val_samples = len(self.validdataset)
        # validation_sampler = RandomSampler(self.validdataset, num_samples=val_samples, replacement=True)
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        multiview_minibatch = torch.stack([i for i in multiview_aug_imgs])

        if self.debug:
            print("Shape of a minibatch")
            print(multiview_minibatch.shape)
            # self.debug = False
    ...
    def sample_episode(self, idx):
        ...
        # Sampled anchor image data
        patch_info_list = self.img_metadata[idx]

        ##############################
        # Details of the anchor sample
        ##############################
        # get the class label
        anchor_class = patch_info_list[0]

        ##############################
        # Metadata of the anchor class
        ##############################
        count_row_indices = len(self.class_metadata[anchor_class][0])
        count_column_indices = len(self.class_metadata[anchor_class][1])
        count_slice_indices = len(self.class_metadata[anchor_class][2])
        ignore_indices_for_a_row = count_column_indices*count_slice_indices
        anchor_sample_indices = self.per_class_sample_min_max_indices[anchor_class]

        ########################################
        # Negative samples from the anchor class
        ########################################
        # get the valid indices in the anchor class
        # that can be used as negative samples
        anchor_volume_row = patch_info_list[2]
        anchor_volume_row_index = self.class_metadata[anchor_class][0].index(anchor_volume_row)
        # print(anchor_class)
        # print(anchor_volume_row)
        # print(self.class_metadata[anchor_class][0])
        # print(anchor_volume_row_index)

        # Ignore top and bottom row around the anchor patch
        patch_overlap = self.data_info[anchor_class]['patch_overlap']
        depth_change_in_pixels = self.patch_size*patch_overlap
        for depth_index in range(-1, 2, 2):
            depth_val = depth_index*depth_change_in_pixels
            cur_depth = anchor_volume_row + depth_val
            if cur_depth < anchor_volume_row:
                index_of_cur_depth = anchor_volume_row_index-1
                start_ignore_indices = max(0, ignore_indices_for_a_row*index_of_cur_depth)
                # print(cur_depth)
                # print(index_of_cur_depth)
                # print(ignore_indices_for_a_row*index_of_cur_depth)
            elif cur_depth > anchor_volume_row:
                index_of_cur_depth = anchor_volume_row_index+1
                # print(cur_depth)
                # print(ignore_indices_for_a_row*(index_of_cur_depth+1))
                # print((ignore_indices_for_a_row*count_row_indices))
                end_ignore_indices = min((ignore_indices_for_a_row*count_row_indices), (ignore_indices_for_a_row*(index_of_cur_depth+1)))
        # adding the starting offset for the anchor class
        # in the image metadata object
        start_ignore_indices += anchor_sample_indices[0]
        end_ignore_indices += anchor_sample_indices[0]
        # print(start_ignore_indices, end_ignore_indices)
        ignore_indices_anchor_class = np.arange(start_ignore_indices, end_ignore_indices).tolist()
        total_indices = np.arange(anchor_sample_indices[0], anchor_sample_indices[1]).tolist()
        valid_anchor_class_indices = list(set(total_indices).difference(ignore_indices_anchor_class))

        # samples per class including the anchor class
        samples_per_class = self.batch_size // len(self.classes)

        # sufficient negative samples not present in the anchor class
        # that satisfies the depth constraint
        # thus adjust the **sample count per class**
        ignore_anchor_class_neg_samples = False
        if len(valid_anchor_class_indices) < samples_per_class:
            print(f"No negative samples present in the anchor class : {anchor_class}")
            ignore_anchor_class_neg_samples = True
            samples_per_class = self.batch_size // (len(self.classes)-1)
        ...
        for class_index, class_name in enumerate(self.classes):
            ...
            if class_index == len(self.classes)-1:
                samples_per_class = self.batch_size - count_samples - 1 # one example in the batch is the anchor sample
            # For classes other than the anchor class
            if class_name != anchor_class:
                ...
                if len(total_class_indices) < samples_per_class:
                    negative_sample_indices = np.random.choice(total_class_indices, samples_per_class, replace=True).tolist()
                else:
                    negative_sample_indices = np.random.choice(total_class_indices, samples_per_class, replace=False).tolist()
            # If there ares sufficient negative samples in the anchor class
            elif class_name == anchor_class:
                if not ignore_anchor_class_neg_samples:
                    negative_sample_indices = np.random.choice(valid_anchor_class_indices, samples_per_class, replace=False).tolist()
                else:
                    continue
        ...
        if self.debug:
            ...
            print("Anchor row index             : " + str(anchor_volume_row_index))
            print("Samples in a row             : " + str(ignore_indices_for_a_row))
            print("Ignore indices (start, end)  : " + str((start_ignore_indices, end_ignore_indices)))
            print("Indices of the image metadata: " + str(minibatch_sample_indices))
    ...
    def read_data(self, minibatch_sample_indices):
        ...
        for index in minibatch_sample_indices:
            ...
            # print('Inside *read_data* function to read patches as PIL images')
            # print('Reading a patch from class   : ' + str(class_name))
            # print('Patch index in the metadata  : ' + str(index))
            # print('Size of the patch            : ' + str(rgb_image.size))
            # print('Number of patches read so far: ' + str(len(image_list)))
...
def show_combined_images(minibatch_data, data_aug, class_names, save_img_path):
    ...
    for img_index in range(num_images):
        minibatch_index = 2*img_index
        img_1 = minibatch_data[minibatch_index, :, :, :]
        ax1 = fig.add_subplot(gs[0, img_index])
        ax1.set_xticklabels([])
        ax1.set_yticklabels([])
        ...
        img_2 = minibatch_data[minibatch_index+1, :, :, :]
        ax2 = fig.add_subplot(gs[1, img_index])
        ax2.set_xticklabels([])
        ax2.set_yticklabels([])
        ax2.set_aspect('equal')
        aug_index = data_aug[minibatch_index+1].cpu().detach().numpy()[0]
...
if __name__ == '__main__':
    ...
    seismic_seg_data_loader = Seismic_Segmentation_Task(classes, data_info, patch_size=256, train_batch_size=train_batch_size, val_batch_size=val_batch_size, debug=set_debug)
    ...
    for i, train_minibatch in enumerate(train_loader):
        data = torch.squeeze(train_minibatch['data'], dim=0)
    ...
    for i, val_minibatch in enumerate(val_loader):
        data = torch.squeeze(val_minibatch['data'], dim=0)
